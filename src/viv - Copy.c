...
// TODO: (*=done)
// fix stream for gifs, don;t load the entire image before loading the gif.
...
static IStream *_viv_istream_create(const wchar_t *filename);
static HRESULT _viv_istream_query_interface(IStream *stream,REFIID riid,void **ppvObject);
static ULONG _viv_istream_add_ref(IStream *stream);
static ULONG _viv_istream_release(IStream *stream);
static HRESULT _viv_istream_read(IStream *stream,void *pv,ULONG cb,ULONG *pcbRead);
static HRESULT _viv_istream_write(IStream *stream,const void *pv,ULONG cb,ULONG *pcbWritten);
static HRESULT _viv_istream_seek(IStream *stream,LARGE_INTEGER dlibMove,DWORD dwOrigin,ULARGE_INTEGER *plibNewPosition);
static HRESULT _viv_istream_set_size(IStream *stream,ULARGE_INTEGER libNewSize);
static HRESULT _viv_istream_copy_to(IStream *stream,IStream *pstm,ULARGE_INTEGER cb,ULARGE_INTEGER *pcbRead,ULARGE_INTEGER *pcbWritten);
static HRESULT _viv_istream_commit(IStream *stream,DWORD grfCommitFlags);
static HRESULT _viv_istream_revert(IStream *stream);
static HRESULT _viv_istream_lock_region(IStream *stream,ULARGE_INTEGER libOffset,ULARGE_INTEGER cb,DWORD dwLockType);
static HRESULT _viv_istream_unlock_region(IStream *stream,ULARGE_INTEGER libOffset,ULARGE_INTEGER cb,DWORD dwLockType);
static HRESULT _viv_istream_stat(IStream *stream,STATSTG *pstatstg,DWORD grfStatFlag);
static HRESULT _viv_istream_clone(IStream *stream,IStream **ppstm);
static void _viv_istream_free(viv_istream_t *stream);
...
// the virtual table.
static const IStreamVtbl _viv_istream_vtbl = 
{
	_viv_istream_query_interface,
	_viv_istream_add_ref,
	_viv_istream_release,
	_viv_istream_read,
	_viv_istream_write,
	_viv_istream_seek,
	_viv_istream_set_size,
	_viv_istream_copy_to,
	_viv_istream_commit,
	_viv_istream_revert,
	_viv_istream_lock_region,
	_viv_istream_unlock_region,
	_viv_istream_stat,
	_viv_istream_clone,
};
...
// 14.447 - CreateStreamOnHGlobal - this is too slow over slow networks.
// 14.520 - CreateStreamOnHGlobal - this is too slow over slow networks.
...
static DWORD WINAPI viv_load_image_proc(void *param)
{
	...
	stream = _viv_istream_create(viv_load_image_filename);
	if (stream)
	{
...
static IStream *_viv_istream_create(const wchar_t *filename)
{
	...
	if (h != INVALID_HANDLE_VALUE)
	{
		viv_istream_t *stream;
		DWORD size;
		
		size = GetFileSize(h,0);
	
		stream = CoTaskMemAlloc(sizeof(viv_istream_t));
		
		if (stream)
		{
			os_zero_memory(stream,sizeof(viv_istream_t));
			
			stream->lpVtbl = &_viv_istream_vtbl;
			stream->h = h;
			stream->size = size;
			stream->ref_count = 1;
			...
			if (size)
			{
				stream->buf = CoTaskMemAlloc(size);
...
static HRESULT _viv_istream_query_interface(IStream *stream,REFIID riid,void **ppvObject)
{
	debug_printf("IStream::QueryInterface\n");	
...
static ULONG _viv_istream_add_ref(IStream *stream)
{
	debug_printf("IStream::AddRef\n");	
	((viv_istream_t *)stream)->ref_count++;

	return ((viv_istream_t *)stream)->ref_count;
}

static ULONG _viv_istream_release(IStream *stream)
{
	debug_printf("IStream::Release\n");	
	
	((viv_istream_t *)stream)->ref_count--;

	if(!((viv_istream_t *)stream)->ref_count)
	{
		debug_printf("stream free\n");
		_viv_istream_free((viv_istream_t *)stream);
...
static HRESULT _viv_istream_read(IStream *stream,void *pv,ULONG cb,ULONG *pcbRead)
{
//	debug_printf("read %d, pos %d, size %d\n",cb,((viv_istream_t *)stream)->pos,((viv_istream_t *)stream)->size);

	if (((viv_istream_t *)stream)->pos < ((viv_istream_t *)stream)->size)
	{
		// clamp cb
		if (((viv_istream_t *)stream)->pos + cb > ((viv_istream_t *)stream)->size)
		{
			cb = ((viv_istream_t *)stream)->size - ((viv_istream_t *)stream)->pos;
		}
		
		// read in buffer.
		if (((viv_istream_t *)stream)->pos + cb <= ((viv_istream_t *)stream)->buf_avail)
		{
//debug_printf("copy buf pos %d, cb %d\n",((viv_istream_t *)stream)->pos,cb);	

			// copy buffer:
			os_copy_memory(pv,((viv_istream_t *)stream)->buf + ((viv_istream_t *)stream)->pos,cb);
		...
		else
		{
			...
			read_end = ((viv_istream_t *)stream)->pos + cb;
			
			// round up to 1MB
			read_end += 0xFFFFF;
			read_end &= ~0xFFFFF;
				
			if (read_end > ((viv_istream_t *)stream)->size)
			{
				read_end = ((viv_istream_t *)stream)->size;
			}
				
			read_size = read_end - ((viv_istream_t *)stream)->buf_avail;
			
//debug_printf("bufavail %d, read end %d, read_size %d\n",((viv_istream_t *)stream)->buf_avail,read_end,read_size);	
				
			if (ReadFile(((viv_istream_t *)stream)->h,((viv_istream_t *)stream)->buf + ((viv_istream_t *)stream)->buf_avail,read_size,&numread,NULL))
			{
				Sleep(1000);
				if (numread == read_size)
				{
					((viv_istream_t *)stream)->buf_avail += read_size;
...
//debug_printf("copy buf pos %d, cb %d\n",((viv_istream_t *)stream)->pos,cb);	
				...
				{
debug_printf("bad numread\n");
				}
			}
			else
			{
debug_printf("bad read %d\n",GetLastError());
			}
		}
	}
...
static HRESULT _viv_istream_write(IStream *stream,const void *pv,ULONG cb,ULONG *pcbWritten)
{
	debug_printf("IStream::Write\n");	
	return E_NOTIMPL;
}

static HRESULT _viv_istream_seek(IStream *stream,LARGE_INTEGER dlibMove,DWORD dwOrigin,ULARGE_INTEGER *plibNewPosition)
{
	ULARGE_INTEGER abs_move;
	
	switch(dwOrigin)
	{
		case STREAM_SEEK_SET:
			abs_move.QuadPart = dlibMove.QuadPart;
			break;
			
		case STREAM_SEEK_CUR:
			abs_move.QuadPart = ((viv_istream_t *)stream)->pos + dlibMove.QuadPart;
			break;
			
		case STREAM_SEEK_END:
			abs_move.QuadPart = ((viv_istream_t *)stream)->size + dlibMove.QuadPart;
			break;
	...
	if (abs_move.QuadPart > 0xffffffffI64)
	{
		return STG_E_INVALIDPOINTER;
	}
	
	((viv_istream_t *)stream)->pos = abs_move.LowPart;
	
//debug_printf("seek origin %d, move %d, abs %d, newpos %d\n",dwOrigin,dlibMove.LowPart,abs_move.LowPart,((viv_istream_t *)stream)->pos);	

	if (plibNewPosition)
	{
		plibNewPosition->QuadPart = ((viv_istream_t *)stream)->pos;
	}
...
static HRESULT _viv_istream_commit(IStream *stream,DWORD grfCommitFlags)
{
	debug_printf("IStream::Commit\n");	
	return E_NOTIMPL;
}

static HRESULT _viv_istream_revert(IStream *stream)
{
	debug_printf("IStream::Revert\n");	
	return E_NOTIMPL;
}

static HRESULT _viv_istream_lock_region(IStream *stream,ULARGE_INTEGER libOffset,ULARGE_INTEGER cb,DWORD dwLockType)
{
	debug_printf("IStream::LockRegion\n");	
	return E_NOTIMPL;
}

static HRESULT _viv_istream_unlock_region(IStream *stream,ULARGE_INTEGER libOffset,ULARGE_INTEGER cb,DWORD dwLockType)
{
	debug_printf("IStream::UnlockRegion\n");	
	return E_NOTIMPL;
}

static HRESULT _viv_istream_stat(IStream *stream,STATSTG *pstatstg,DWORD grfStatFlag)
{
	debug_printf("IStream::Stat\n");	

	os_zero_memory(pstatstg,sizeof(STATSTG));
	pstatstg->cbSize.QuadPart = ((viv_istream_t *)stream)->size;
...
static HRESULT _viv_istream_clone(IStream *stream,IStream **ppstm)
{
	debug_printf("IStream::Clone\n");	
	return E_NOTIMPL;
}

static void _viv_istream_free(viv_istream_t *stream)
{
	if (((viv_istream_t *)stream)->buf)
	{
		CoTaskMemFree(((viv_istream_t *)stream)->buf);
	}
	
	CoTaskMemFree(stream);
}
